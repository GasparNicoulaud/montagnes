...
void main( void )
{
    ...
    */
    ...
    if(voronoiNoise(vec3(gl_FragCoord.xy + voroWarping,brseedAbs + 1) * voroFreq,voroPeriod).z * 0.8 - 0.10 > height)
    {
    ...
    if(voronoiNoise(vec3(gl_FragCoord.xy + voroWarping,brseedAbs + 2) * voroFreq,voroPeriod).z * 0.4 + 0.14 > height)
    {
    ...
    float smoothnessValue = pow(abs(rand1dTo1d(rockID + brseedAbs + 1)),1.5) * 0.8;
    float colorModulator = pow(abs(rand1dTo1d(rockID + brseedAbs + 5)),3);
