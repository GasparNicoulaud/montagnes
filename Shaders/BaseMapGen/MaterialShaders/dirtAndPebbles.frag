...
void main( void )
{
    ...
    */
    ...
    voroFreq = 1.0/16.0;
    voroPeriod = vec3(128);
    voroWarping = vec2(FBMPeriodic(gl_FragCoord.xy / 2048.0) * 64 - 120,FBMPeriodic(gl_FragCoord.xy / 2048.0) * 64 - 120) * 8.0;
    if(voronoiNoise(vec3(gl_FragCoord.xy + voroWarping,brseedAbs + 6) * voroFreq,voroPeriod).z * 0.03 + 0.375 > height)
    {
        rockID = voronoiNoise(vec3(gl_FragCoord.xy + voroWarping,brseedAbs + 6) * voroFreq,voroPeriod).y;
        //rockID = 5;
    }
    height = max(voronoiNoise(vec3(gl_FragCoord.xy + voroWarping,brseedAbs + 6) * voroFreq,voroPeriod).z * 0.03 + 0.375,height);
    voroFreq = 1.0/8.0;
    voroPeriod = vec3(512);
    voroWarping = vec2(FBMPeriodic(gl_FragCoord.xy / 2048.0) * 128 + 140,FBMPeriodic(gl_FragCoord.xy / 2048.0) * 128 + 140) * 4.0;
    if(voronoiNoise(vec3(gl_FragCoord.xy + voroWarping,brseedAbs + 7) * voroFreq,voroPeriod).z * 0.015 + 0.381 > height)
    {
        rockID = voronoiNoise(vec3(gl_FragCoord.xy + voroWarping,brseedAbs + 7) * voroFreq,voroPeriod).y;
        //rockID = 6;
    }
    height = max(voronoiNoise(vec3(gl_FragCoord.xy + voroWarping,brseedAbs + 7) * voroFreq,voroPeriod).z * 0.015 + 0.381,height);
    voroFreq = 1.0/4.0;
    voroPeriod = vec3(1024);
    voroWarping = vec2(FBMPeriodic(gl_FragCoord.xy / 2048.0) * 256 - 160,FBMPeriodic(gl_FragCoord.xy / 2048.0) * 256 - 160) * 2.0;
    if(voronoiNoise(vec3(gl_FragCoord.xy + voroWarping,brseedAbs + 8) * voroFreq,voroPeriod).z * 0.008 + 0.385 > height)
    {
        rockID = voronoiNoise(vec3(gl_FragCoord.xy + voroWarping,brseedAbs + 8) * voroFreq,voroPeriod).y;
        //rockID = 7;
    }
    height = max(voronoiNoise(vec3(gl_FragCoord.xy + voroWarping,brseedAbs + 8) * voroFreq,voroPeriod).z * 0.008 + 0.385,height);
    
    float dirtFloor = FBMPeriodicWithFreq(gl_FragCoord.xy / 2048.0  + vec2(brseedAbs + 1), 10.0) * 0.015;
    ...
    float smoothnessValue = pow(abs(rand1dTo1d(rockID + brseedAbs + 1)),1.2) * 0.8;
    float colorModulator = pow(abs(rand1dTo1d(rockID + brseedAbs + 5)),5);
    
    if(height > dirtFloor)
    {
        albedoHeight.rgb = pebblesColor * colorModulator + pebblesColor;
    }
    else
    {
        smoothnessValue *= 0.1;
        albedoHeight.rgb = dirtColor * highFreqPerlinNoise * 100.0 + dirtColor;
    }
    
    height = max(height,dirtFloor);
