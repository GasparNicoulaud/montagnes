...
void main( void )
{
    ...
    float rockID = 0;
    float voroScale;
    float voroSeed = brseedAbs;
    
    
    voroSeed +=1;
    voroScale = 4.0;
    voroFreq = 1.0/voroScale;
    voroPeriod = vec3(512);
    voroWarping = vec2(FBMPeriodic(gl_FragCoord.xy / 2048.0) * voroScale + 100,FBMPeriodic(gl_FragCoord.xy / 2048.0) * voroScale + 100) * 8.0;
    if(voronoiNoise(vec3(gl_FragCoord.xy + voroWarping,brseedAbs + voroSeed) * voroFreq,voroPeriod).z * 0.008 + 0.365 > height)
    {
        rockID = voronoiNoise(vec3(gl_FragCoord.xy + voroWarping,brseedAbs + voroSeed) * voroFreq,voroPeriod).y;
        //rockID = 1;
    }
    height = max(voronoiNoise(vec3(gl_FragCoord.xy + voroWarping,brseedAbs + voroSeed) * voroFreq,voroPeriod).z * 0.008 + 0.365,height);
    voroSeed +=1;
    voroScale = 5.0;
    voroFreq = 1.0/voroScale;
    voroPeriod = vec3(512);
    ...
    float perlinNoise = FBMPeriodicWithFreq(gl_FragCoord.xy / 2048.0  + vec2(brseedAbs + 2), 1.0) * 0.02;
    float highFreqPerlinNoise = FBMPeriodic(gl_FragCoord.xy * 0.005 + vec2(brseedAbs + 3)) * 5.0;;
    
    height += perlinNoise * 0.15;
    
    float smoothnessValue = pow(abs(rand1dTo1d(rockID + brseedAbs + 1)),1.2) * 0.8;
    float colorModulator = pow(abs(rand1dTo1d(rockID + brseedAbs + 5)),0.5);
    
    albedoHeight.rgb = vec3(0.139+ highFreqPerlinNoise,0.122+ highFreqPerlinNoise,0.117+ highFreqPerlinNoise) * colorModulator * mainColor;
