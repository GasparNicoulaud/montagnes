...
void main(void)
{
    ...
    vec2 randGradient = vec2(cos(brseedAbs),sin(brseedAbs)) * sin(brseedAbs * 123.456) * 0.001;
    
    float terrainHeight = FBMWithRotations(gl_FragCoord.xy * globalFreq,brseedAbs) * globalAmp * 0.5;
    
    terrainHeight += dot(gl_FragCoord.xy,randGradient);
    
    terrainHeight = mod(terrainHeight, 0.05 + abs(cos(brseedAbs * 789.1011)) * abs(sin(brseedAbs * 136.101)));
    
    terrainHeight += FBMWithRotations(gl_FragCoord.xy * globalFreq,brseedAbs + 12) * globalAmp * 0.5;
    
    out_Color = vec4(terrainHeight,0,0,1.0);
}
