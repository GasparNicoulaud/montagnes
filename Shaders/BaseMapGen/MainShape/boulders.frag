...
float boulderFBMWithRotations(vec2 pos, float seed)
{
    pos *= 0.01;
    pos += vec2(cos(seed * 2),sin(seed * -3)) * 10;
    ...
    for (int i = 0; i < 13; i++) {
        ...
        float noiseval = sqrt(max(snoise(pos * pow(lacunarity,i)) - 0.4f,0.0f));
        retval += noiseval / pow(lacunarity,i);
    }
    return sqrt(retval + 3.0) - 1.5;
}
...
void main(void)
{
    ...
    //bouldersNoise = min(sqrtPlainsNoise,0.5 * globalAmp);
