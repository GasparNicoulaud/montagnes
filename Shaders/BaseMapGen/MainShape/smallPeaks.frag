...
float perlinMinField(vec2 pos, float seed)
{
    float height = 999.0;
    
    height = min(height,FBMWithRotations(pos,seed + 0));
    height = min(height,FBMWithRotations(pos,seed + 1));
    height = min(height,FBMWithRotations(pos,seed + 2));
    height = min(height,FBMWithRotations(pos,seed + 3));
    height = min(height,FBMWithRotations(pos,seed + 4));
    height = min(height,FBMWithRotations(pos,seed + 5));
    height = min(height,FBMWithRotations(pos,seed + 6));
    height = min(height,FBMWithRotations(pos,seed + 7));
...
void main(void)
{
    ...
    float bouldersNoise = -999.0;
    
    bouldersNoise = perlinMinField(gl_FragCoord.xy * globalFreq,brseedAbs) * globalAmp + offset;
    
    globalAmp *= 0.5;
    offset *= 0.5;
    globalFreq *= 2.0;
    bouldersNoise = max(bouldersNoise, perlinMinField(gl_FragCoord.xy * globalFreq,brseedAbs) * globalAmp) + offset;
    ...
    out_Color = vec4(bouldersNoise,0,0,1.0);
}
