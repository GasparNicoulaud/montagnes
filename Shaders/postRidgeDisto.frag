...
{
    float freq = 4;
    vec2 perturbation = vec2(basicFBM(gl_FragCoord.xy * freq,123),basicFBM(gl_FragCoord.xy * freq,-321)) * 10;
    //perturbation = vec2(0);
    
    float outc = texture(sampler,(gl_FragCoord.xy + perturbation)/2048.0f).r;
    
    float addedNoise = basicFBM(gl_FragCoord.xy,546) * 0.2;
