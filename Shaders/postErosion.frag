...
{
    float freq = 50;
    vec2 perturbation = vec2(basicFBM(gl_FragCoord.xy * freq,33),basicFBM(gl_FragCoord.xy * freq,-991)) * 1;
    //perturbation = vec2(0);
    
    vec4 outc = texture(sampler,(gl_FragCoord.xy + perturbation)/2048.0f).rgba;
    
    float addedNoise = basicFBM(gl_FragCoord.xy,-123) * 0.02;
    outc += addedNoise;
